...
export default function SignupPage() {
  ...
  async function handleForgotPassword() {
    ...
    const { error: resetErr } = await supabase.auth.resetPasswordForEmail(email, {
      // Keep it simple: send user back to the login page after reset.
    ...
    <div className="flex min-h-screen flex-col items-center justify-center bg-zinc-50 px-4 dark:bg-zinc-950">
      <div className="w-full max-w-sm rounded-lg border border-zinc-200 bg-white p-6 shadow dark:border-zinc-800 dark:bg-zinc-900">
        ...
        <div className="mt-4 border-t border-zinc-200 pt-4 dark:border-zinc-800">
          {resetError && <p className="rounded bg-red-50 px-3 py-2 text-sm text-red-700 dark:bg-red-900/30 dark:text-red-300">{resetError}</p>}
          {resetMessage && (
          ...
          <p className="mt-2 text-center text-xs text-zinc-500 dark:text-zinc-400">
            Uses your email above. You’ll receive a reset link by email.
          </p>
        </div>
